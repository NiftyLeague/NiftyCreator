...
namespace GraphQlClient.Core
{
    ...
    public class GraphApi : ScriptableObject
    {
        ...
        void HandleIntrospection(){
            ...
            File.WriteAllText($"{Application.dataPath}{dsc}Prefabs{dsc}GraphQL{dsc}{name}schema.txt",introspection);
        ...
        public void GetSchema(){
            if (schemaClass == null){
                try{
                    char dsc = Path.DirectorySeparatorChar;
                    introspection = File.ReadAllText($"{Application.dataPath}{dsc}Prefabs{dsc}GraphQL{dsc}{name}schema.txt");
                }
                catch{
                    return;
                }
