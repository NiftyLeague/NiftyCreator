...
public class ConstantsGeneratorKit : MonoBehaviour
{
	...
	private const string NAMESPACE = "NiftyLeague";
	private static ConstantNamingStyle CONSTANT_NAMING_STYLE = ConstantNamingStyle.CamelCase;
	private const string DIGIT_PREFIX = "No";
	private static string[] IGNORE_RESOURCES_IN_SUBFOLDERS = new string[] { };
	...
	private static string getClassContent(string className, string[] labelsArray)
	{
		...
		if (className == SCENES_FILE_NAME.Replace(".cs", string.Empty))
		{
			...
			output += "\t\t{\n";
			output += "\t\t\tif( SceneManager.GetActiveScene().buildIndex + 1 == " + TOTAL_SCENES_CONSTANT_NAME + " )\n";
			output += "\t\t\t\treturn 0;\n";
			output += "\t\t\treturn SceneManager.GetActiveScene().buildIndex + 1;\n";
			output += "\t\t}\n";
		}
