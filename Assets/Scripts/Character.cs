...
public class Character : MonoBehaviour
{
	...
	private const float timebumpMultiplier = 0.175f;
	...
	public void Die(EffectsController.Side? side = null)
	{
	...
	void Hit(Character hitChar, Vector2 hitDir)
	{
		...
		characterEvents.Hit(hitChar, hitDir, attackChargeM + ingestPowerBoost);
	}
	...
	public void GetHitInit(Vector2 hitDir, int hitsTaken, float power, Character attacker)
	{
		...
		EffectsController.CreateLocalizedShake(transform.position + Vector3.up * height * 0.5f, velocity, velocity.magnitude, timeBump * timebumpMultiplier);
		EffectsController.CreateHitEffect(transform.position + Vector3.up * height * 0.5f, timeBump * timebumpMultiplier, power >= 1f);
		if (power >= 1f)
		{
	...
	private void RunSpinDash()
	{
		...
		case SpinDashState.Spinning:
			...
			{
				var cols = Physics2D.OverlapCircleAll((Vector2)transform.position, spinDashRadius, characterLayer);
				foreach (var col in cols)
				{
					var chr = col.GetComponent<Character>();
					if (chr != null && chr != this && !spinDashHitPlayers.Contains(chr.player))
					{
	...
	private void RunTeleport()
	{
		...
		case TeleportState.Teleporting:
			...
			EffectsController.CreateTeleportSolo(teleportPosition + new Vector3(0f, 1.5f, -2f));
			teleportState = TeleportState.Idle;
			state = wasBouncingBeforeSpecial ? CharacterState.Bouncing : CharacterState.Normal;
			break;
		}
	...
	private void RunThrow()
	{
		if (throwDelayLeft <= 0f && throwState == ThrowState.Charging)
		{
			...
			else if (input.wasBButton)
			{
				...
				throwable.Initialize(this, specialAttackDir * speed, transform.position + (Vector3.up * height / 1.5f));
	...
	{
		if (pounceState != PounceState.Idle && pounceState != PounceState.Active)
		{
			return false;
		}
		specialAttackDir = GetSpecialDirection();
	...
	{
		if (spinDashState != SpinDashState.Idle)
		{
			return false;
		}
		spinDashHitPlayers.Clear();
	...
	private bool StartTeleportAttack()
	{
		...
		EffectsController.CreateTeleportMewtwo(transform.position + new Vector3(0f, 1.5f, -2f));
		return true;
	}
